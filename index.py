import cv2
import mediapipe as mp
import os
from pygame import mixer

# Initialize Audio
mixer.init()
audio_path = "1.mp3"
if os.path.exists(audio_path):
    mixer.music.load(audio_path)
else:
    print(f"Warning: {audio_path} not found. Run the ffmpeg command first!")

# Setup MediaPipe
mp_face_mesh = mp.solutions.face_mesh
face_mesh = mp_face_mesh.FaceMesh(min_detection_confidence=0.5, min_tracking_confidence=0.5)

#Setup Video Sources
video_path = "1.mov"
video_cap = cv2.VideoCapture(video_path)
cap = cv2.VideoCapture(1) # Your C270 Webcam

video_window_name = "GET BACK TO WORK"
window_open = False

while cap.isOpened():
    success, frame = cap.read()
    if not success: break

    frame = cv2.flip(frame, 1)
    h, w, _ = frame.shape
    results = face_mesh.process(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

    looking_away = True 

    if results.multi_face_landmarks:
        nose = results.multi_face_landmarks[0].landmark[4]
        # Focus Box (0.4 to 0.6 is the middle 20% of the screen feel free to play around with the values ;)
        if 0.40 < nose.x < 0.60 and 0.35 < nose.y < 0.65:
            looking_away = False

    if looking_away:
        # --- Handle Video ---
        ret, v_frame = video_cap.read()
        if not ret:
            video_cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
            ret, v_frame = video_cap.read()
        
        if ret:
            cv2.imshow(video_window_name, v_frame)
            window_open = True
        
        # --- Handle Audio ---
        if not mixer.music.get_busy(): # If music isn't already playing
            mixer.music.play(-1)      # Loop indefinitely while looking away
    else:
        # Stop everything when focused
        if window_open:
            cv2.destroyWindow(video_window_name)
            mixer.music.stop()
            window_open = False

    # Status Preview
    status_color = (0, 0, 255) if looking_away else (0, 255, 0)
    cv2.putText(frame, f"LOOKING AWAY: {looking_away}", (10, 30), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, status_color, 2)
    cv2.imshow("Webcam Preview", frame)

    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

cap.release()
video_cap.release()
cv2.destroyAllWindows()
mixer.quit()
